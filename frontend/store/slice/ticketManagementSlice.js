...
import {
  ...
  putTicketData,
  postTicketData,
} from "../../service/data-service/ticketDataService";
import {
  getAllTickets,
  getRowFiltering,
  fillDropdownValues,
...
export const ticketManagementSlice = createSlice({
  ...
  reducers: {
    ...
    saveSelectedFilter: (state, action) => {
      ...
      state.filteredRows = getRowFiltering(
        state.fetchedRows,
    ...
    defaultFilterDropdown: (state, action) => {
      state.dropDownValues = fillDropdownValues(state.fetchedRows, {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateTicketDataAsync.fulfilled, (state, action) => {
        let _index = state.fetchedRows.findIndex(
          (x) => x.id === action.payload.id
        );
        const { firstname, lastname } = state.users.find(
          (x) => x.id === action.payload.requester_id
        );
      ...
      .addCase(fetchRows.fulfilled, (state, action) => {
        ...
        action.payload.forEach((x, i) => {
          ...
          row["id"] = id;
          row["customer"] =
            `${name?.firstname} ${name?.lastname}` || "No Contact";
          row["subject"] = subject;
          row["agent"] =
            `${responderName?.firstname} ${responderName?.lastname}` ||
            "Unassigned"; // create logic for assignee .. get from requester id
          row["state"] = dueInfo[0];
          row["dueDate"] = dueInfo[1];
          row["status"] = status;
          row["priority"] = priority;
          // row['group'] = 'group 1 ';
          row["ticketTypes"] = assignTicketTypes(
            x,
            dueInfo[0],
            dueInfo[1],
            row["agent"]
        ...
        state.filteredRows = getRowFiltering(finalFormat, {
      ...
      .addCase(getAllUserDetails.fulfilled, (state, action) => {
        ...
        state.dropDownValues.agent = action.payload
          .filter((user) => user.role === "Agent")
          .map((x) => ({ name: `${x.firstname} ${x.lastname}`, id: x.id }));
        state.dropDownValues.customer = action.payload
